...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  document.querySelectorAll('form.builder-form').forEach(form=>{
    ...
    const textarea=form.querySelector('textarea[name="content"]'),type=form.querySelector('select[name="type"]'),moduleId=form.querySelector('input[name="module_id"]').value;
    ...
    const hidden=document.createElement('input');hidden.type='hidden';hidden.name='r2_content';const preview=document.createElement('div');preview.className='r2-video-preview';preview.hidden=true;preview.innerHTML='<div><b>▶ Aperçu de la vidéo</b><small>La vidéo est privée et ce lien est temporaire.</small></div><video controls preload="metadata"></video>';
    textarea.after(hidden,upload,progress,preview);const input=upload.querySelector('input'),bar=progress.querySelector('b'),label=progress.querySelector('span'),video=preview.querySelector('video');
    const showPreview=(url,content)=>{hidden.value=content;textarea.value=content;video.src=url;preview.hidden=false;progress.hidden=false;bar.style.width='100%';label.textContent='✓ Vidéo prête. Vérifiez l’aperçu puis cliquez sur « Créer la leçon ».';button.textContent='✓ Créer la leçon';};
    const syncType=()=>{const isVideo=type.value==='video';upload.hidden=!isVideo;preview.hidden=!isVideo||!hidden.value;textarea.placeholder=isVideo?'Ou collez ici une URL YouTube / vidéo':'Saisissez le contenu de la leçon';};type.addEventListener('change',syncType);syncType();
    input.addEventListener('change',async()=>{const file=input.files[0];if(!file)return;progress.hidden=false;bar.style.width='2%';label.textContent='Création du lien sécurisé…';button.disabled=true;try{const response=await fetch('/admin/r2/video-url',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({course_id:form.querySelector('input[name="course_id"]').value,name:file.name,type:file.type,size:file.size})});const data=await response.json();if(!response.ok||!data.ok)throw new Error(data.message||'Configuration R2 indisponible.');await new Promise((resolve,reject)=>{const xhr=new XMLHttpRequest();xhr.open('PUT',data.upload_url);xhr.timeout=30*60*1000;xhr.setRequestHeader('Content-Type',file.type);xhr.upload.onprogress=event=>{if(event.lengthComputable){const value=Math.round(event.loaded/event.total*100);bar.style.width=value+'%';label.textContent='Envoi vers R2 : '+value+'%';}};xhr.onload=()=>xhr.status>=200&&xhr.status<300?resolve():reject(new Error('R2 a refusé le fichier (HTTP '+xhr.status+').'));xhr.onerror=()=>reject(new Error('Connexion R2 bloquée. Vérifiez le bucket et sa politique CORS.'));xhr.ontimeout=()=>reject(new Error('Le délai d’envoi a expiré.'));xhr.onabort=()=>reject(new Error('Envoi annulé.'));xhr.send(file);});showPreview(data.preview_url,data.content);}catch(error){bar.style.width='0';label.textContent=error.message;input.value='';}finally{button.disabled=false;}});
    form.addEventListener('submit',event=>{if(button.disabled){event.preventDefault();label.textContent='Attendez la fin de l’envoi de la vidéo.';return;}if(type.value==='video'&&!hidden.value&&!textarea.value.trim()){event.preventDefault();progress.hidden=false;label.textContent='Choisissez une vidéo R2 ou collez une URL avant de continuer.';}});lessonForms.set(moduleId,{form,type,textarea,hidden,preview,showPreview,syncType});
  });
  if(lessonForms.size)fetch('/admin/cours/brouillons-lecons').then(response=>response.json()).then(data=>{Object.entries(data.drafts||{}).forEach(([moduleId,draft])=>{const ui=lessonForms.get(String(moduleId));if(!ui)return;ui.form.querySelector('input[name="title"]').value=draft.title||'';ui.type.value=draft.type||'video';ui.form.querySelector('input[name="duration"]').value=draft.duration||10;ui.textarea.value=draft.content||'';if((draft.content||'').startsWith('r2://')&&draft.preview_url)ui.showPreview(draft.preview_url,draft.content);ui.syncType();ui.form.closest('details').open=true;});}).catch(()=>{});
});
