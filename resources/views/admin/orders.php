...
<header class="orders-heading"><div><p class="orders-eyebrow">GESTION COMMERCIALE</p><h1>Commandes</h1><p>Suivez vos ventes, organisez les livraisons et gérez les demandes clients.</p></div><a class="order-btn order-btn-outline" href="/admin/documents/export-commandes"><svg data-icon="grid" aria-hidden="true"></svg> Exporter</a></header>
...
<div class="orders-toolbar"><label class="orders-search"><svg data-icon="search" aria-hidden="true"></svg><input type="search" data-order-search placeholder="Référence, client, téléphone, transaction…" aria-label="Rechercher une commande"></label><label class="orders-filter">Statut<select data-order-filter><option value="all">Toutes les commandes</option><option value="active">À traiter</option><option value="paid">Payées</option><option value="completed">Livrées / terminées</option><option value="cancelled">Annulées</option><option value="refund">Remboursement demandé</option><option value="refunded">Remboursées</option><option value="rejected">Remboursement refusé</option></select></label><span data-order-count aria-live="polite"><?= count($items) ?> commande(s)</span></div>
...
$search=implode(' ',[$order['reference'],$order['customer']??'',$order['email']??'',$order['phone']??'',$order['refund']['reference']??'',$order['refund']['reason']??'',...array_map(fn($p)=>implode(' ',[$p['transaction_reference']??'',$p['provider_reference']??'',$p['payer_phone']??'']),$order['payments'])]);
...
?>
<article class="order-card <?= $closed?'is-cancelled':'' ?>" data-order-card data-search="<?= $escape($search) ?>" data-status="<?= $escape($order['status']) ?>" data-payment="<?= $escape($order['payment_status']) ?>" data-refund-status="<?= $escape($order['refund']['status']??'') ?>" data-paid="<?= $paid?'1':'0' ?>" data-refund="<?= ($order['refund']['status']??'')==='requested'?'1':'0' ?>">
<header class="order-card-header"><div class="order-identity"><span class="order-symbol" aria-hidden="true"><svg data-icon="<?= $hasPhysical?'grid':'book' ?>" aria-hidden="true"></svg></span><div><h2><?= $escape($order['reference']) ?></h2><p><?= $escape(date('d/m/Y à H:i',strtotime($order['created_at']))) ?> · <?= $hasPhysical?'Commande boutique':'Formation / numérique' ?></p></div></div><div class="order-badges"><span class="order-badge logistics-<?= $escape($order['status']) ?>"><?= $escape($logistics[$order['status']]??$order['status']) ?></span><span class="order-badge payment-<?= $escape($order['payment_status']) ?>"><?= $escape(CommerceDetails::label($order['payment_status'])) ?></span></div></header>
...
<?php endif ?>
<?php if($paid&&(float)$order['total']>0&&(empty($order['refund'])||$order['refund']['status']==='rejected'))$button('request_refund','Demander un remboursement'); ?>
<?php if(($order['refund']['status']??'')==='requested'&&$paid){$button('complete_refund','Confirmer le remboursement');$button('reject_refund','Refuser la demande');} ?>
...
<?php if(!empty($order['refund'])): $refund=$order['refund']; ?><aside class="order-refund-summary"><div><span class="order-badge payment-refunded"><?= $escape(CommerceDetails::label($refund['status'])) ?></span><strong><?= number_format((float)$refund['amount'],0,',',' ') ?> FCFA</strong></div><p>Demandé le <?= $escape(date('d/m/Y à H:i',strtotime($refund['requested_at']))) ?> · <?= $escape($refund['reason']) ?></p><?php if($refund['reference']): ?><p>Référence : <b><?= $escape($refund['reference']) ?></b> · <?= $escape($refund['method']) ?> · Exécuté le <?= $escape($refund['completed_at']) ?></p><?php endif ?><?php if($refund['admin_note']): ?><p><?= $escape($refund['admin_note']) ?></p><?php endif ?></aside><?php endif ?>
<div class="order-details-grid"><details class="order-disclosure"><summary><svg data-icon="wallet" aria-hidden="true"></svg> Paiement et justificatifs</summary><div><?php require __DIR__.'/../partials/payment-details.php'; ?><div class="order-documents"><a class="order-btn order-btn-outline" href="/admin/documents/commande?id=<?= (int)$order['id'] ?>" target="_blank" rel="noopener">Bon de commande ↗</a><?php if($hasPhysical): ?><a class="order-btn order-btn-outline" href="/admin/documents/livraison?id=<?= (int)$order['id'] ?>" target="_blank" rel="noopener">Bon de livraison ↗</a><?php endif ?></div></div></details><details class="order-disclosure"><summary><svg data-icon="clock" aria-hidden="true"></svg> Historique <span><?= count($order['history']) ?></span></summary><ol class="order-timeline"><?php foreach($order['history'] as $event): ?><li><time><?= $escape($event['created_at']) ?></time><strong><?= $escape($logistics[$event['status']]??$event['status']) ?></strong><p><?= $escape($event['note']??'') ?></p></li><?php endforeach ?><?php if(!$order['history']): ?><li>Aucun événement enregistré.</li><?php endif ?></ol></details></div>
</article>
<?php endforeach ?>
</div>
<div class="orders-empty" data-order-empty <?= $items?'hidden':'' ?>><svg data-icon="search" aria-hidden="true"></svg><h2><?= $items?'Aucune commande trouvée':'Aucune commande pour le moment' ?></h2><p>Les commandes de la boutique et des formations apparaissent ici.</p><button class="order-btn order-btn-outline" type="button" data-order-reset>Réinitialiser les filtres</button></div>
<a href="/admin/commandes/statut" data-order-status-url hidden></a><a href="/admin/commandes/action" data-order-action-url hidden></a>
<dialog class="order-dialog" data-order-dialog aria-labelledby="order-dialog-title" aria-describedby="order-dialog-description">
<form method="post" data-order-form action="/admin/commandes/statut">
