...
final class LiveSessionService
{
    ...
    public function participantUrl(array $session,string $userName,bool $owner=false): string
    {
        if(($session['provider']??'')!=='daily')return (string)$session['room_url'];$settings=new SecureSettings();$key=$settings->get('daily_api_key');if($key==='')throw new \RuntimeException('Clé API Daily non configurée.');$exp=time()+7200;if(!empty($session['starts_at'])){$t=strtotime((string)$session['starts_at']);if($t)$exp=max(time()+1800,$t+7200);}$payload=['properties'=>['room_name'=>$session['room_name'],'user_name'=>$userName,'is_owner'=>$owner,'exp'=>$exp,'enable_screenshare'=>true]];$ch=curl_init('https://api.daily.co/v1/meeting-tokens');curl_setopt_array($ch,[CURLOPT_POST=>true,CURLOPT_RETURNTRANSFER=>true,CURLOPT_HTTPHEADER=>['Authorization: Bearer '.$key,'Content-Type: application/json'],CURLOPT_POSTFIELDS=>json_encode($payload),CURLOPT_TIMEOUT=>20,CURLOPT_SSL_VERIFYPEER=>true]);$body=curl_exec($ch);$code=(int)curl_getinfo($ch,CURLINFO_RESPONSE_CODE);curl_close($ch);$data=json_decode((string)$body,true);if($code<200||$code>=300||empty($data['token']))throw new \RuntimeException('Impossible de générer le jeton de réunion Daily.');return rtrim((string)$session['room_url'],'/').'?t='.rawurlencode((string)$data['token']);
    }
    private function daily(SecureSettings $settings,string $slug,string $label,?string $startsAt): array
    {
        $key=$settings->get('daily_api_key');if($key==='')throw new \RuntimeException('Clé API Daily non configurée.');$exp=time()+86400;if($startsAt){$t=strtotime($startsAt);if($t)$exp=$t+7200;}$payload=['name'=>$slug,'privacy'=>'private','properties'=>['exp'=>$exp,'enable_screenshare'=>true,'enable_chat'=>true,'start_video_off'=>false,'start_audio_off'=>false]];$ch=curl_init('https://api.daily.co/v1/rooms');curl_setopt_array($ch,[CURLOPT_POST=>true,CURLOPT_RETURNTRANSFER=>true,CURLOPT_HTTPHEADER=>['Authorization: Bearer '.$key,'Content-Type: application/json'],CURLOPT_POSTFIELDS=>json_encode($payload),CURLOPT_TIMEOUT=>20,CURLOPT_SSL_VERIFYPEER=>true]);$body=curl_exec($ch);$code=(int)curl_getinfo($ch,CURLINFO_RESPONSE_CODE);curl_close($ch);$data=json_decode((string)$body,true);if($code<200||$code>=300||empty($data['url']))throw new \RuntimeException('Impossible de créer la salle Daily.');return ['provider'=>'daily','room_name'=>$data['name']??$slug,'room_url'=>$data['url'],'room_data'=>json_encode($data,JSON_UNESCAPED_SLASHES)];
    }
}
