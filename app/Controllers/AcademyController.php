...
{
    private function requireCourseAccess(int $courseId): void
    {
        $user=$this->user(); if (($user['role']??'')==='admin') return;
        $stmt=Database::connection()->prepare("SELECT 1 FROM enrollments WHERE user_id=? AND course_id=? AND status IN ('active','completed')");
    ...
    public function course(): void
    {
        ...
        $stmt=$db->prepare("SELECT * FROM enrollments WHERE user_id=? AND course_id=? AND status IN ('active','completed')");$stmt->execute([$user['id'],$courseId]);$enrollment=$stmt->fetch();
