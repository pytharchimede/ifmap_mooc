...
final class AdminController
{
    ...
    public function products(): void { $this->guard(); $items=Database::connection()->query("SELECT id,name title,reference,category,CASE stock_status WHEN 'in_stock' THEN 'En stock' WHEN 'on_order' THEN 'Sur commande' ELSE 'Rupture' END stock,price,promotional_price,status,image FROM products ORDER BY id DESC")->fetchAll();View::render('admin/resources',['title'=>'Produits','active'=>'admin-products','type'=>'products','items'=>$items],'admin'); }
    ...
    {
        $this->guard();$id=(int)($_POST['id']??0);$name=mb_substr(trim((string)($_POST['name']??'')),0,190);if($name===''){$_SESSION['flash']='Le nom du produit est obligatoire.';$this->redirect($id?'/admin/produits/modifier?id='.$id:'/admin/produits/nouveau');}$db=Database::connection();$current=null;if($id){$stmt=$db->prepare('SELECT image,gallery FROM products WHERE id=?');$stmt->execute([$id]);$current=$stmt->fetch();}$images=$this->uploadProductImages($current?:[], $id?'/admin/produits/modifier?id='.$id:'/admin/produits/nouveau');$slug=$this->uniqueProductSlug($db,$name,$id);$price=($_POST['price']??'')===''?null:max(0,(float)$_POST['price']);$promo=($_POST['promotional_price']??'')===''?null:max(0,(float)$_POST['promotional_price']);if($promo!==null&&$price!==null&&$promo>=$price)$promo=null;$values=[$name,$slug,mb_substr(trim((string)($_POST['reference']??'')),0,80)?:'IFMAP-'.strtoupper(substr(bin2hex(random_bytes(4)),0,8)),mb_substr(trim((string)($_POST['category']??'Équipements')),0,120),trim((string)($_POST['description']??'')),$price,$promo,$_POST['stock_status']??'in_stock',max(0,(int)($_POST['stock_quantity']??0)),mb_substr(trim((string)($_POST['lead_time']??'')),0,100),$images['image'],json_encode($images['gallery']),in_array($_POST['status']??'',['draft','published','archived'],true)?$_POST['status']:'draft'];try{if($id){$values[]=$id;$db->prepare('UPDATE products SET name=?,slug=?,reference=?,category=?,description=?,price=?,promotional_price=?,stock_status=?,stock_quantity=?,lead_time=?,image=?,gallery=?,status=? WHERE id=?')->execute($values);}else{$db->prepare('INSERT INTO products(name,slug,reference,category,description,price,promotional_price,stock_status,stock_quantity,lead_time,image,gallery,status) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)')->execute($values);$id=(int)$db->lastInsertId();}$_SESSION['flash']='Produit enregistré avec succès.';$this->redirect('/admin/produits/modifier?id='.$id);}catch(\Throwable $e){$_SESSION['flash']='Enregistrement impossible : vérifiez notamment que la référence est unique.';$this->redirect($id?'/admin/produits/modifier?id='.$id:'/admin/produits/nouveau');}
    }
    ...
    public function updateOrderStatus(): void { $this->guard();$id=(int)($_POST['id']??0);$status=in_array($_POST['status']??'',['pending','paid','processing','shipping','completed','cancelled'],true)?$_POST['status']:'pending';$db=Database::connection();$db->beginTransaction();try{$stmt=$db->prepare('SELECT status FROM orders WHERE id=? FOR UPDATE');$stmt->execute([$id]);$previous=$stmt->fetchColumn();if(!$previous)throw new \RuntimeException('Commande absente');$db->prepare('UPDATE orders SET status=?,payment_status=CASE WHEN ?="paid" THEN "paid" WHEN ?="cancelled" AND payment_status="paid" THEN "refunded" ELSE payment_status END WHERE id=?')->execute([$status,$status,$status,$id]);if($status==='cancelled'&&$previous!=='cancelled'){$items=$db->prepare("SELECT item_id,quantity FROM order_items WHERE order_id=? AND item_type='product'");$items->execute([$id]);$restore=$db->prepare("UPDATE products SET stock_quantity=stock_quantity+?,stock_status=CASE WHEN stock_status='out_of_stock' THEN 'in_stock' ELSE stock_status END WHERE id=?");foreach($items as $item)$restore->execute([(int)$item['quantity'],(int)$item['item_id']]);}$db->prepare('INSERT INTO order_status_history(order_id,status,note) VALUES(?,?,?)')->execute([$id,$status,mb_substr(trim((string)($_POST['note']??'')),0,255)]);$db->commit();$_SESSION['flash']='Statut de commande mis à jour.';}catch(\Throwable){if($db->inTransaction())$db->rollBack();$_SESSION['flash']='Impossible de modifier cette commande.';}$this->redirect('/admin/commandes'); }
    ...
    private function renderOrderDocument(string $type): void { $this->guard();$id=(int)($_GET['id']??0);$db=Database::connection();$stmt=$db->prepare('SELECT * FROM orders WHERE id=?');$stmt->execute([$id]);$order=$stmt->fetch();if(!$order){http_response_code(404);return;}$stmt=$db->prepare('SELECT * FROM order_items WHERE order_id=? ORDER BY id');$stmt->execute([$id]);$items=$stmt->fetchAll();if($type==='delivery')$items=array_values(array_filter($items,fn($item)=>$item['item_type']==='product'));View::render('admin/order-document',['title'=>$type==='delivery'?'Bon de livraison':'Bon de commande','order'=>$order,'items'=>$items,'documentType'=>$type,'brand'=>$this->documentBrand($db)],'document'); }
    ...
    {
        $gallery=json_decode((string)($current['gallery']??'[]'),true)?:[];$primary=$current['image']??null;if(empty($_FILES['images']['tmp_name'])||!is_array($_FILES['images']['tmp_name']))return ['image'=>$primary,'gallery'=>$gallery];$allowed=['image/jpeg'=>'jpg','image/png'=>'png','image/webp'=>'webp'];$directory=dirname(__DIR__,2).'/public/uploads/products';if(!is_dir($directory)&&!mkdir($directory,0775,true)&&!is_dir($directory)){$_SESSION['flash']='Impossible de créer le dossier produits.';$this->redirect($redirectPath);}foreach($_FILES['images']['tmp_name'] as $index=>$tmp){if(!$tmp||!is_uploaded_file($tmp))continue;$size=(int)($_FILES['images']['size'][$index]??0);$mime=(new \finfo(FILEINFO_MIME_TYPE))->file($tmp);if(!isset($allowed[$mime])||$size>6*1024*1024){$_SESSION['flash']='Image invalide : JPG, PNG ou WebP, 6 Mo maximum.';$this->redirect($redirectPath);}$filename='product-'.date('YmdHis').'-'.bin2hex(random_bytes(6)).'.'.$allowed[$mime];if(!move_uploaded_file($tmp,$directory.'/'.$filename)){$_SESSION['flash']='Impossible d’enregistrer une image.';$this->redirect($redirectPath);}$gallery[]='/public/uploads/products/'.$filename;} $gallery=array_values(array_unique($gallery));if(!$primary&&$gallery)$primary=$gallery[0];return ['image'=>$primary,'gallery'=>$gallery];
    }
