...
final class AdminController
{
    ...
    public function courses(): void { $this->guard(); $courses=Database::connection()->query("SELECT c.id,c.title,c.category sector,c.mode,c.duration_label duration,c.price,c.tone,c.status,c.thumbnail,COUNT(DISTINCT m.id) module_count,COUNT(DISTINCT l.id) lesson_count,COUNT(DISTINCT e.user_id) learner_count FROM courses c LEFT JOIN modules m ON m.course_id=c.id LEFT JOIN lessons l ON l.module_id=m.id LEFT JOIN enrollments e ON e.course_id=c.id GROUP BY c.id ORDER BY c.id DESC")->fetchAll(); View::render('admin/courses', ['title'=>'Gestion des cours','active'=>'admin-courses','courses'=>$courses], 'admin'); }
    ...
    public function activateUser(): void { $this->guard();$id=(int)($_POST['id']??0);$db=Database::connection();$db->prepare("UPDATE users SET status='active',phone_verified_at=COALESCE(phone_verified_at,NOW()),otp_code=NULL,otp_expires_at=NULL WHERE id=?")->execute([$id]);$db->prepare("UPDATE enrollments SET status='active' WHERE user_id=? AND status='pending' AND payment_status IN ('paid','free')")->execute([$id]);$_SESSION['flash']='Compte et inscriptions activés.';$this->redirect('/admin/utilisateurs'); }
    ...
    public function updateUser(): void
    {
        ...
        $db->beginTransaction();try{$sql='UPDATE users SET name=?,email=?,phone=?,role=?,status=?,otp_code='.($status==='active'?'NULL':'otp_code').',otp_expires_at='.($status==='active'?'NULL':'otp_expires_at').($password!==''?',password=?':'').' WHERE id=?';$values=[$name,$email,$phone,$role,$status];if($password!=='')$values[]=password_hash($password,PASSWORD_DEFAULT);$values[]=$id;$db->prepare($sql)->execute($values);if($status==='active')$db->prepare("UPDATE enrollments SET status='active' WHERE user_id=? AND status='pending' AND payment_status IN ('paid','free')")->execute([$id]);$db->commit();$_SESSION['flash']=$password!==''?'Compte mis à jour et nouveau mot de passe défini.':'Compte mis à jour.';}catch(\Throwable){if($db->inTransaction())$db->rollBack();$_SESSION['flash']='La mise à jour du compte a échoué.';}$this->redirect('/admin/utilisateurs/modifier?id='.$id);
    }
    public function enrollments(): void { $this->guard();$items=Database::connection()->query("SELECT e.enrolled_at,e.status,e.progress,e.payment_status,o.reference order_reference,u.name,u.email,u.phone,c.title course_title FROM enrollments e LEFT JOIN orders o ON o.id=e.order_id JOIN users u ON u.id=e.user_id JOIN courses c ON c.id=e.course_id ORDER BY e.enrolled_at DESC")->fetchAll();View::render('admin/enrollments',['title'=>'Inscriptions aux cours','active'=>'admin-enrollments','items'=>$items],'admin'); }
    ...
    private function commerceGuard(): void
    {
        $this->guard();
        if (!is_string($_POST['csrf'] ?? null) || !hash_equals($_SESSION['commerce_csrf'] ?? '', $_POST['csrf']) || empty($_SESSION['commerce_csrf'])) { http_response_code(403); exit; }
    }
    ...
    {
        $this->commerceGuard(); $db=Database::connection(); $id=(int)($_POST['id']??0);
        $service=new \App\Services\OrderLifecycle($db);
        $note=mb_substr(trim((string)($_POST['note']??'')),0,2000);
        $reference=mb_substr(trim((string)($_POST['reference']??'')),0,190);
        $method=mb_substr(trim((string)($_POST['method']??'')),0,80);
        try {
            switch ($_POST['action'] ?? '') {
                ...
                case 'collect_cod':
                    ...
                    if (!$order || $order['payment_status']!=='cod' || $order['status']==='cancelled' || $reference==='' || $method==='') throw new \RuntimeException('Commande à encaisser, référence de reçu et moyen de paiement requis.');
                    (new SiteController())->finalizePaidOrder($id,['data'=>['operator_id'=>$reference,'payment_method'=>$method,'payer_phone'=>mb_substr(trim((string)($_POST['payer_phone']??'')),0,40)]],'cash_on_delivery');
                    break;
    ...
    {
        $this->guard(); $db=Database::connection();
        $rows=$db->query("SELECT o.*,JSON_UNQUOTE(JSON_EXTRACT(o.customer_data,'$.name')) client,JSON_UNQUOTE(JSON_EXTRACT(o.customer_data,'$.email')) email,JSON_UNQUOTE(JSON_EXTRACT(o.customer_data,'$.phone')) telephone FROM orders o ORDER BY o.id DESC")->fetchAll();
        header('Content-Type: text/csv; charset=UTF-8');header('Content-Disposition: attachment; filename=commandes-ifmap-'.date('Y-m-d').'.csv');
        $out=fopen('php://output','w');fwrite($out,"\xEF\xBB\xBF");
        fputcsv($out,['Commande','Client','Email','Téléphone contact','Logistique','État paiement','Total FCFA','Prestataire','Référence transaction','Référence session','Téléphone payeur','Canal','Payé le','Livré le','Retour reçu le','Remboursement','Montant remboursé','Référence remboursement','Remboursé le'],';');
        foreach($rows as $row) {
            $payments=\App\Services\CommerceDetails::payments($db,(int)$row['id']);$refund=\App\Services\CommerceDetails::refund($db,(int)$row['id']);
            foreach($payments?:[[]] as $payment) {
                $cells=[$row['reference'],$row['client'],$row['email'],$row['telephone'],$row['status'],$row['payment_status'],$row['total'],$payment['provider']??'',$payment['transaction_reference']??'',$payment['provider_reference']??'',$payment['payer_phone']??'',$payment['payment_channel']??'',$payment['paid_at']??'',$row['delivered_at'],$row['returned_at'],$refund['status']??'',($refund['status']??'')==='completed'?$refund['amount']:0,$refund['reference']??'',$refund['completed_at']??''];
                $cells=array_map(fn($v)=>preg_match('/^[=+@\-\t\r]/',(string)$v)?"'".$v:$v,$cells);
                fputcsv($out,$cells,';');
            }
        }
        fclose($out);exit;
    }
    public function exportEnrollments(): void { $this->guard();$rows=Database::connection()->query("SELECT u.name,u.email,u.phone,c.title,e.status,e.progress,e.enrolled_at,e.completed_at FROM enrollments e LEFT JOIN orders o ON o.id=e.order_id JOIN users u ON u.id=e.user_id JOIN courses c ON c.id=e.course_id ORDER BY e.enrolled_at DESC")->fetchAll();header('Content-Type: text/csv; charset=UTF-8');header('Content-Disposition: attachment; filename=inscriptions-ifmap-'.date('Y-m-d').'.csv');$out=fopen('php://output','w');fwrite($out,"\xEF\xBB\xBF");fputcsv($out,['Apprenant','Email','Téléphone','Formation','Statut','Progression %','Inscription','Fin'],';');foreach($rows as $row)fputcsv($out,array_values($row),';');fclose($out);exit; }
