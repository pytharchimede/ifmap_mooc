...
final class SiteController
{
    ...
    public function placeOrder(): void
    {
        ...
        $db=Database::connection();$db->beginTransaction();try{
            ...
            $downloads=[];foreach($_SESSION['cart'] as $key=>$item){$itemId=($item['kind']??'')==='formation'?(int)($item['course_id']??0):(int)($item['product_id']??0);$itemType=($item['kind']??'')==='formation'?'course':'product';$itemStmt->execute([$orderId,$itemType,$itemId,$item['name'],$item['quantity'],$item['price'],$item['price']*$item['quantity']]);$orderItemId=(int)$db->lastInsertId();if($itemType==='product'&&$itemId&&($item['product_type']??'physical')==='physical'){$stockStmt=$db->prepare('SELECT stock_quantity FROM products WHERE id=? FOR UPDATE');$stockStmt->execute([$itemId]);$before=(int)$stockStmt->fetchColumn();$after=max(0,$before-(int)$item['quantity']);$db->prepare("UPDATE products SET stock_quantity=?,stock_status=CASE WHEN ?=0 THEN 'out_of_stock' ELSE stock_status END WHERE id=?")->execute([$after,$after,$itemId]);$db->prepare("INSERT INTO stock_movements(product_id,order_id,movement_type,quantity,stock_before,stock_after,note) VALUES(?,?,'sale',?,?,?,'Vente en ligne')")->execute([$itemId,$orderId,-(int)$item['quantity'],$before,$after]);}if($itemType==='product'&&($item['product_type']??'')==='digital'&&$paymentStatus==='paid'){$rawToken=bin2hex(random_bytes(32));$db->prepare('INSERT INTO digital_downloads(order_id,order_item_id,product_id,token_hash,download_limit,expires_at) VALUES(?,?,?,?,?,?)')->execute([$orderId,$orderItemId,$itemId,hash('sha256',$rawToken),max(1,(int)($item['download_limit']??5)),date('Y-m-d H:i:s',strtotime('+30 days'))]);$downloads[]=['name'=>$item['name'],'token'=>$rawToken,'limit'=>max(1,(int)($item['download_limit']??5))];}if($itemType==='course'&&$userId&&$paymentStatus==='paid'&&$itemId){$db->prepare("INSERT INTO enrollments(user_id,course_id,status,source) VALUES(?,?,'active','purchase') ON DUPLICATE KEY UPDATE status='active'")->execute([$userId,$itemId]);}}
            ...
            $db->commit();$order=['id'=>$orderId,'reference'=>$reference,'customer'=>$customer['name'],'email'=>$customer['email'],'payment'=>$_POST['payment'],'items'=>$_SESSION['cart'],'downloads'=>$downloads,'total'=>$total,'discount'=>$discount,'status'=>$status,'created_at'=>date('c')];
