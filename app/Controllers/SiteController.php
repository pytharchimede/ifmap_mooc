...
final class SiteController
{
    ...
    public function placeOrder(): void
    {
        ...
        if($_POST['payment']==='online'&&$total>0){try{$gateway=new CinetPay();if(!$gateway->configured())throw new \RuntimeException('Configuration CinetPay absente.');$dbOrder=$db->query('SELECT * FROM orders WHERE id='.$orderId)->fetch();$response=$gateway->initialize($dbOrder,$this->publicUrl('/paiement/cinetpay/retour'),$this->publicUrl('/paiement/cinetpay/notification'));$paymentUrl=(string)($response['data']['payment_url']??'');$token=(string)($response['data']['payment_token']??'');if((string)($response['code']??'')!=='201'||!filter_var($paymentUrl,FILTER_VALIDATE_URL))throw new \RuntimeException('Initialisation refusée.');$db->prepare("INSERT INTO payments(order_id,provider,provider_reference,amount,status,payload) VALUES(?,'cinetpay',?,?,'initiated',?)")->execute([$orderId,$token,$total,json_encode($response,JSON_UNESCAPED_UNICODE)]);$_SESSION['pending_order_id']=$orderId;$_SESSION['cart']=[];unset($_SESSION['coupon']);header('Location: '.$paymentUrl);exit;}catch(\Throwable $e){$db->prepare("UPDATE orders SET payment_status='failed' WHERE id=?")->execute([$orderId]);$_SESSION['checkout_error']='Le guichet CinetPay est temporairement indisponible. Votre panier a été conservé, veuillez réessayer.';$this->redirect('/commande');}}
    ...
    {
        $transactionId=trim((string)($_REQUEST['transaction_id']??$_REQUEST['cpm_trans_id']??''));$orderId=(int)($_SESSION['pending_order_id']??0);
    ...
    {
        $db=Database::connection();$stmt=$db->prepare('SELECT id,total FROM orders WHERE reference=? LIMIT 1');$stmt->execute([$transactionId]);$order=$stmt->fetch();if(!$order)throw new \RuntimeException('Transaction inconnue.');
        $verification=(new CinetPay())->check($transactionId);$data=is_array($verification['data']??null)?$verification['data']:$verification;$status=strtoupper((string)($data['status']??''));$amount=(float)($data['amount']??-1);$currency=strtoupper((string)($data['currency']??'XOF'));
        if(!in_array($status,['ACCEPTED','SUCCESS'],true)||abs($amount-(float)$order['total'])>0.001||$currency!=='XOF'){$db->prepare("UPDATE payments SET status='failed',payload=? WHERE order_id=? AND provider='cinetpay'")->execute([json_encode($verification,JSON_UNESCAPED_UNICODE),(int)$order['id']]);$db->prepare("UPDATE orders SET payment_status='failed' WHERE id=? AND payment_status<>'paid'")->execute([(int)$order['id']]);return (int)$order['id'];}
