...
final class SiteController
{
    ...
    public function placeOrder(): void
    {
        ...
        if($_POST['payment']==='online'&&$total>0){try{$gateway=new CinetPay();if(!$gateway->configured())throw new \RuntimeException('Configuration CinetPay absente.');$dbOrder=$db->query('SELECT * FROM orders WHERE id='.$orderId)->fetch();$response=$gateway->initialize($dbOrder,$this->publicUrl('/paiement/cinetpay/retour'),$this->publicUrl('/paiement/cinetpay/notification'));$paymentUrl=(string)($response['data']['payment_url']??'');$providerReference=(string)($response['data']['transaction_id']??$response['data']['payment_token']??'');if((string)($response['code']??'')!=='201'||!filter_var($paymentUrl,FILTER_VALIDATE_URL))throw new \RuntimeException('Initialisation refusée : '.(string)($response['message']??$response['description']??'réponse non reconnue').'.');$db->prepare("INSERT INTO payments(order_id,provider,provider_reference,amount,status,payload) VALUES(?,'cinetpay',?,?,'initiated',?)")->execute([$orderId,$providerReference,$total,json_encode($response,JSON_UNESCAPED_UNICODE)]);$_SESSION['pending_order_id']=$orderId;$_SESSION['cart']=[];unset($_SESSION['coupon']);header('Location: '.$paymentUrl);exit;}catch(\Throwable $e){error_log('CinetPay init commande '.$orderId.' : '.$e->getMessage());$db->prepare("UPDATE orders SET payment_status='failed' WHERE id=?")->execute([$orderId]);$network=str_contains(strtolower($e->getMessage()),'resolve')||str_contains(strtolower($e->getMessage()),'network')||str_contains(strtolower($e->getMessage()),'timeout');$_SESSION['checkout_error']=$network?'Le serveur ne parvient pas à joindre CinetPay (connexion Internet ou DNS indisponible). Votre panier a été conservé.':'Le guichet CinetPay a refusé l’initialisation. Votre panier a été conservé ; veuillez réessayer ou contacter IFMAP.';$this->redirect('/commande');}}
